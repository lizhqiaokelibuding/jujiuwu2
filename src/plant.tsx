...
export default function Farm({ day, setDay, hasDrawnToday, setHasDrawnToday, hand, setHand, equippedTool, setEquippedTool, money, setMoney, mapCards, setMapCards, onGoToIzakaya }: FarmProps) {
  ...
  return (
    <div className="fixed inset-0 w-full h-full overflow-hidden bg-[#a8d5a2] p-4">
      <div className="relative w-full h-full rounded-2xl overflow-hidden border-8 border-[#d4b483] shadow-inner">
      
      {/* Background Image - 农场.png，若无则显示绿色背景 */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{ 
          backgroundImage: `url("/backgrounds/${encodeURIComponent('农场.png')}")`,
      ...
      {/* 顶部状态栏 - 第几天 + 多少钱 */}
      <header className="absolute top-4 left-0 right-0 flex justify-center items-center gap-8 z-20 pointer-events-auto">
        <div className="bg-[#f4e4bc] border-4 border-[#d4b483] px-8 py-3 rounded-2xl shadow-lg">
          <span className="text-2xl font-bold text-amber-900">农场第{day}天</span>
        </div>
        <div className="bg-[#f4e4bc] border-4 border-[#d4b483] px-8 py-3 rounded-2xl shadow-lg flex items-center gap-2">
          <span className="text-2xl">💰</span>
          <span className="text-2xl font-bold text-amber-900">{money}</span>
        </div>
        <button 
          onClick={drawCards}
          disabled={hasDrawnToday}
          className={`px-6 py-3 rounded-2xl font-bold text-lg border-4 shadow-lg ${
            hasDrawnToday 
              ? 'bg-gray-300 border-gray-400 text-gray-600 cursor-not-allowed' 
              : 'bg-amber-200 border-[#d4b483] text-amber-900 hover:bg-amber-300'
          }`}
        >
        ...
        {/* 装备栏 - 紧凑 */}
        <div 
          className="bg-[#f4e4bc] border-4 border-[#d4b483] rounded-2xl p-2 min-w-[80px] flex items-center justify-center"
          onDragOver={e => e.preventDefault()}
          onDrop={handleEquipDrop}
        >
          {equippedTool ? (
            <div 
              className="w-12 h-12 flex items-center justify-center text-3xl cursor-pointer hover:opacity-80"
              onClick={() => { addCardToHand(equippedTool.type as CardType, 1); setEquippedTool(null); }}
              title="点击卸下"
            >
              {equippedTool.image ? <img src={equippedTool.image} alt="" className="w-10 h-10 object-contain" /> : equippedTool.emoji}
            </div>
          ) : (
            <span className="text-xs font-bold text-amber-800">装备</span>
          )}
        </div>
      </header>
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        <button className="bg-[#f4e4bc] border-4 border-[#d4b483] px-6 py-4 rounded-2xl font-bold text-amber-900 shadow-lg bg-amber-100/80">
          农场
        </button>
        <button 
          onClick={onGoToIzakaya}
          className="bg-[#f4e4bc] border-4 border-[#d4b483] px-6 py-4 rounded-2xl font-bold text-amber-900 hover:bg-[#faeac2] shadow-lg transition-colors"
        >
          居酒屋
        </button>
        <button 
      ...
      {/* 底部卡牌 - 扑克式扇形 */}
      <div className="absolute bottom-0 left-0 right-0 h-56 flex justify-center items-end pb-6 z-40">
